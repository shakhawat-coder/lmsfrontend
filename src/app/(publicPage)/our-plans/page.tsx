  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <section className="relative pt-40 pb-32 overflow-hidden bg-slate-950">
        ...
        />
        <div className="absolute inset-0 z-[1] bg-gradient-to-b from-slate-950/80 via-slate-950/60 to-white"></div>
      ...
      <section className="py-24 bg-white">
        <div className="container px-4 mx-auto">
          <div className="mb-16 text-center">
            <h2 className="mb-4 text-3xl font-bold text-slate-900 md:text-4xl">Why Choose BookNest?</h2>
          ...
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
            {features.map((feature, index) => (
              <motion.div
                ...
                className="p-8 transition-all duration-300 border border-slate-100 bg-slate-50/50 rounded-3xl hover:shadow-xl hover:bg-white active:scale-95 group"
              >
      ...
      <section className="py-24 bg-slate-50">
        <div className="container max-w-4xl px-4 mx-auto">
          <div className="flex items-center justify-center gap-3 mb-12">
            ...
            <h2 className="text-3xl font-bold text-slate-900 md:text-4xl">Common Questions</h2>
          </div>
            ...
            <Accordion type="single" collapsible className="w-full">
              {faqs.map((faq, index) => (
                <AccordionItem
                  key={index}
                  value={`item-${index}`}
                  className={`px-8 border-slate-100 ${index === faqs.length - 1 ? 'border-b-0' : ''}`}
                >
                  <AccordionTrigger className="py-6 text-left hover:no-underline font-bold text-slate-900 text-lg group">
                    {faq.question}
                  </AccordionTrigger>
